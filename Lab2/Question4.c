...
void sortDatabySelection(struct Q4Struct array[], int size)
{
    ...
    int i = 0, j = 0, minPos = 0, intTemp = 0;
    char charTemp;
    for(i = 0; i < size; i++){
        minPos = i;
        for(j = i+1; j < size; j++){
            if(array[j].intData < array[minPos].intData){
                minPos = j;
            }
        }
        intTemp = array[i].intData;
        array[i].intData = array[minPos].intData;
        array[minPos].intData = intTemp;
        charTemp = array[i].charData;
        array[i].charData = array[minPos].charData;
        array[minPos].charData = charTemp;
    }
